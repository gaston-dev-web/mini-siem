...
Detection is only half the job — an alert nobody can act on has achieved nothing.
[`docs/incident-notes/`](docs/incident-notes/) contains worked triage writeups for
alerts this SIEM raised: what fired, what the raw logs actually showed, whether it
was a true positive, and what to do next. The data is synthetic (generated by the
included attack simulator), but the triage process is the real one.
